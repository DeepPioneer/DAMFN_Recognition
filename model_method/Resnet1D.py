...
class Res1dNet18(nn.Module):
    def __init__(self, num_classes):
        super(Res1dNet18, self).__init__()
        self.conv0 = nn.Conv1d(in_channels=1, out_channels=64, kernel_size=11, stride=5, padding=5, bias=False)
        self.bn0 = nn.BatchNorm1d(64)
        self.resnet = _ResNetWav1d(_ResnetBasicBlockWav1d, [2, 2, 2, 2])

        self.avgpool = nn.AdaptiveAvgPool1d(output_size=(1))
        self.flatten = nn.Flatten()
        self.fc1 = nn.Linear(512, num_classes, bias=True)
...
if __name__ == "__main__":
    ...
    model = Res1dNet18(num_classes=5)
    output = model(x)
    print(output.shape)
